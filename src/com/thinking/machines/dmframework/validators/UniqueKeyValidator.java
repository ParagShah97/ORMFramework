...
public class UniqueKeyValidator implements KeyValidator {
    ...
    public void validate(Connection connection, Object object, boolean throwExceptionIfExists)
        ...
        for (UniqueKeyWrapper uniqueKeyWrapper : uniqueKeyWrappers) {
            validateUniqueKey(connection, object, throwExceptionIfExists, uniqueKeyWrapper.getGetterMethods(),
                    uniqueKeyWrapper.getPreparedStatementSetterMethods(), uniqueKeyWrapper.getExceptionMessage(),
                    uniqueKeyWrapper.getSQLStatement(), uniqueKeyWrapper.hasCompositeKey(), validatorException);
        }
    ...
    public void validateBeforeInsertion(Connection connection, Object object, boolean throwExceptionIfExists)
        ...
        ValidatorException validatorException = new ValidatorException();
        for (UniqueKeyWrapper uniqueKeyWrapper : uniqueKeyWrappers) {
            validateUniqueKeyBeforeInsertion(connection, object, throwExceptionIfExists,
                    uniqueKeyWrapper.getGetterMethods(),
                    uniqueKeyWrapper.getPreparedStatementSetterMethods(), uniqueKeyWrapper.getExceptionMessage(),
                    uniqueKeyWrapper.getSQLStatement(), uniqueKeyWrapper.hasCompositeKey(), validatorException);
        }
        if (validatorException.hasExceptions())
    ...
    private void validateUniqueKey(Connection connection, Object object, boolean throwExceptionIfExists,
            ArrayList<MethodWrapper> getterMethods, ArrayList<Method> preparedStatementSetterMethods,
            String exceptionMessage, String sqlStatement, boolean hasCompositeKey,
            ValidatorException validatorException) throws DMFrameworkException {
    ...
    private void validateUniqueKeyBeforeInsertion(Connection connection, Object object, boolean throwExceptionIfExists,
            ArrayList<MethodWrapper> getterMethods, ArrayList<Method> preparedStatementSetterMethods,
            String exceptionMessage, String sqlStatement, boolean hasCompositeKey,
            ValidatorException validatorException) throws DMFrameworkException, ValidatorException {
        // if autoincrement is enabled then no need to check
        MethodWrapper methodWrapper = null;
        for (int k = 0; k < getterMethods.size(); k++) {
            methodWrapper = getterMethods.get(k);
            if (methodWrapper.getColumn().getIsAutoIncrementEnabled())
                return;
        }
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
